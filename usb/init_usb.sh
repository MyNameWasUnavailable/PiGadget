#!/bin/sh
#
# inits USB gadget according to config (setup.cfg) set variable for USB gadget directory
...
wdir=$( cd $(dirname $BASH_SOURCE[0]) && cd .. && pwd)
	...
	mkdir -p $GADGETS_DIR
	cd $GADGETS_DIR
	# configure gadget details ========================= set Vendor ID echo 0xc1cb > idVendor # RNDIS
	echo $USB_VID > idVendor # RNDIS
	# set Product ID echo 0xbaa2 > idProduct # RNDIS
...
#	echo "deadbeefdeadbe11" > strings/0x409/serialnumber
	...
	#	an registry hive, the first time a device without OS descriptors is attached. The key is build like this:
	#
	#	HKLM\SYSTEM\CurrentControlSet\Control\usbflags\[USB_VID+USB_PID+bcdRelease\osvc
	#
	#	To allow Windows to read the OS descriptors again, the according registry hive has to be deleted manually or USB descriptor values have to be cahnged (f.e. USB_PID).
		mkdir -p os_desc
		echo 1 > os_desc/use
